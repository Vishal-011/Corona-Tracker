...
var chart3 = new CanvasJS.Chart("chartContainer3", {
	...
	theme: "light2",
	title:{
		text: "Data-Wise Infection Growth"
	},
	axisY:{
		includeZero: false
	},
...
var chart2 = new CanvasJS.Chart("chartContainer2", {
	...
	data: [
		<?php
		while(($row = $result5->fetch_row())!== null)
		{
			$d=0;
			$state = $row['0'];
			echo"\n";
			$query3 = "SELECT count(*),`Date Announced` from `patients` where `State code`= '$state' GROUP BY `Date Announced` ORDER by COUNT(*)";
			$result4 = $conn->query($query3) or die($conn->error);
		echo "{";
			echo "\n";
			echo 'type:"line",';
			echo "\n";
			...
			echo "markerSize: 0,";
			echo "\n";
			echo 'yValueFormatString: "$#,###k",';
			echo "\n";
			echo "dataPoints: [";		
				while(($row1 = $result4->fetch_row())!== null)
				{
					echo "{ x: new Date(2014, 00, 01), y: '{$row1['0']}' },";
					$d= $d+1;
				}
			echo "]";
			echo "\n";
